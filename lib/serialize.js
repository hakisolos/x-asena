...
async function serialize(msg, conn) {
  ...
  if (msg.key) {
    msg.id = msg.key.id;
    msg.raw = msg;
    const jidOwner = jidNormalizedUser(conn.user.id)
    const lidOwner = jidNormalizedUser(conn.user.lid)
    ...
    msg.sender = msg.isGroup
      ...
      ? conn.user.id
      : msg.from;
    msg.isSelf = [jidOwner, lidOwner].includes(msg.sender);
    try {
      msg.sudo =
      ...
      return await conn.sendMessage(msg.key.remoteJid, { text }, { quoted: msg.raw });
	  };
