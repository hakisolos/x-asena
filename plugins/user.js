...
const { PausedChats, WarnDB } = require("../assets/database");
...
command(
  ...
  async (message, match, m) => {
    if (!match) return await message.reply("undefined");
    try {
      const result = await (async () => {
        return eval(`(async () => { ${match} })()`);
      })();
      const output =
        result === undefined ? "undefined" : util.inspect(result, { depth: 1 });
      await message.reply(output);
    } catch (e) {
      await message.reply(util.inspect(e));
    }
  }
);
